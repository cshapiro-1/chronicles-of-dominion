...
@export var max_zoom: float = 70.0
@export var min_pitch: float = 32.0   # Positive degrees above horizon
...
var target_position: Vector3 = Vector3(0.0, 0.0, 8.0)
var target_zoom: float = 34.0
var target_yaw: float = 45.0

func _ready() -> void:
	global_position = target_position
	camera.position = Vector3(0, 0, target_zoom)
	pitch_arm.rotation_degrees.x = -min_pitch
	gimbal.rotation_degrees.y = target_yaw
...
func _process(delta: float) -> void:
	...
	var target_pitch_val = lerp(min_pitch, max_pitch, zoom_factor)
	
	# Negative rotation on X lifts camera into positive Y and aims downward at ground
	pitch_arm.rotation_degrees.x = lerp(pitch_arm.rotation_degrees.x, -target_pitch_val, damping * delta)
